...
import static org.opensearch.neuralsearch.mappingtransformer.SemanticMappingUtils.getUniqueModelIds;
...
public class SemanticFieldProcessor extends AbstractBatchingProcessor {
    ...
    private void fetchModelInfoThenBatchProcess(
        ...
        if (modelIdsToGetConfig.isEmpty()) {
            ...
            final int[] counter = { modelIdsToGetConfig.size() };
            for (String modelId : modelIdsToGetConfig) {
                ...
                }, e -> { handleBatchExecuteException(docToSemanticFieldInfoMap.keySet(), ingestDocumentWrappers, e, handler); }));
            }
        }
    }
    ...
    private void batchProcess(
        ...
        for (Map.Entry<IngestDocumentWrapper, List<SemanticFieldInfo>> entry : docToSemanticFieldInfoMap.entrySet()) {
            try {
                ...
                handleBatchExecuteException(Set.of(entry.getKey()), ingestDocumentWrappers, e, handler);
            }
        }
    ...
    private void handleBatchExecuteException(
        @NonNull final Set<IngestDocumentWrapper> impactedDocs,
        @NonNull final List<IngestDocumentWrapper> docsInRequest,
