...
public class NeuralQueryParser {
    ...
    public static Map<String, NeuralQueryTargetFieldConfig> indexToTargetFieldConfigStreamInput(@NonNull final StreamInput in)
        throws IOException {
        if (!in.readBoolean()) {
            return null;
        } else {
            ...
            final int size = in.readVInt();
            for (int i = 0; i < size; i++) {
                final String indexName = in.readString();
                final NeuralQueryTargetFieldConfig.NeuralQueryTargetFieldConfigBuilder builder = NeuralQueryTargetFieldConfig.builder();
                builder.isSemanticField(in.readOptionalBoolean());
                builder.isUnmappedField(in.readOptionalBoolean());
                builder.searchModelId(in.readOptionalString());
                builder.embeddingFieldType(in.readOptionalString());
                indexToTargetFieldConfigMap.put(indexName, builder.build());
            }
            return indexToTargetFieldConfigMap;
        }
    }

    public static void indexToTargetFieldConfigStreamOutput(
        @NonNull final StreamOutput out,
        final Map<String, NeuralQueryTargetFieldConfig> indexToTargetFieldConfig
    ) throws IOException {
        if (indexToTargetFieldConfig == null) {
            out.writeBoolean(false);
        } else {
            int size = indexToTargetFieldConfig.size();
            out.writeInt(size);
            ...
            for (Map.Entry<String, NeuralQueryTargetFieldConfig> entry : indexToTargetFieldConfig.entrySet()) {
                ...
                out.writeOptionalBoolean(targetFieldConfig.getIsSemanticField());
                out.writeOptionalBoolean(targetFieldConfig.getIsUnmappedField());
                out.writeOptionalString(targetFieldConfig.getSearchModelId());
                out.writeOptionalString(targetFieldConfig.getEmbeddingFieldType());
            }
        }
    }
}
