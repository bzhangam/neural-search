...
public class SemanticFieldProcessorTests extends OpenSearchTestCase {
    ...
    public void testExecute_whenValidDoc_thenIngestDocSuccessfully() throws URISyntaxException, IOException {
        ...
        semanticFieldProcessor.execute(ingestDocument, (doc, e) -> {
            ...
            verify(mlCommonsClientAccessor, times(2)).getModel(any(), any());
    ...
    public void testSubBatchExecute_whenModelNotFound_thenAddExceptionToDocProperly() throws URISyntaxException, IOException {
        ...
        doAnswer(invocationOnMock -> {
            ...
            listener.onFailure(mockModelNotFoundException);
            return null;
    ...
    public void testSubBatchExecute_whenInferenceFail_thenAddExceptionToDocProperly() throws URISyntaxException, IOException {
        // prepare ingest doc 1
        final Map<String, Object> ingestDocSource1 = readDocSourceFromFile("processor/semantic/ingest_doc1.json");
        final IngestDocumentWrapper ingestDocumentWrapper1 = createIngestDocWrapper("1", ingestDocSource1);
        // prepare ingest doc 2
        ...
        // mock generate embedding for dense model fail
        final String exceptionMsg1 = "Failed to inference by dense model.";
        doAnswer(invocationOnMock -> {
            ...
            listener.onFailure(new RuntimeException(exceptionMsg1));
            return null;
        }).when(mlCommonsClientAccessor).inferenceSentences(any(), any());

        // mock generate embedding for sparse model
        final String exceptionMsg2 = "Failed to inference by sparse model.";
        doAnswer(invocationOnMock -> {
            final TextInferenceRequest textInferenceRequest = invocationOnMock.getArgument(0);
            final String modelId = textInferenceRequest.getModelId();
            ...
            listener.onFailure(new RuntimeException(exceptionMsg2));
            return null;
        ...
        // verify
        assertTrue(ingestedDocs.get(0).getException().getMessage().contains(exceptionMsg1));
        assertTrue(ingestedDocs.get(0).getException().getMessage().contains(exceptionMsg2));
        assertTrue(ingestedDocs.get(1).getException().getMessage().contains(exceptionMsg1));
        assertTrue(ingestedDocs.get(1).getException().getMessage().contains(exceptionMsg2));
    }
